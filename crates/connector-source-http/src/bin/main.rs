...
use connector_common::fluvio::RecordKey;
...
use connector_source_http::formatter::HttpResponseRecord;
use tokio_stream::StreamExt;

type Result<T, E = Box<dyn std::error::Error + Send + Sync + 'static>> = core::result::Result<T, E>;

use connector_common::opt::GetOpts;
use connector_source_http::error::Error;
use connector_source_http::HttpOpt;
...
async fn main() -> Result<()> {
    ...
    while timer_stream.next().await.is_some() {
        ...
        let mut formatter =
            HttpResponseRecord::try_from(&response).map_err(|e| Error::Record(e))?;
