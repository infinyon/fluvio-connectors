...
fn main() -> Result<(), MqttConnectorError> {
    ...
    let fluvio_client = fluvio_future::task::run_block_on(fluvio::Fluvio::connect())?;
    let producer = fluvio_future::task::run_block_on(fluvio_client.topic_producer(fluvio_topic))?;

    for msg in rx.iter() {
        if let Some(msg) = msg {
            let mqtt_topic = msg.topic();
            let mqtt_payload = msg.payload();
            fluvio_future::task::run_block_on(producer.send(mqtt_topic, mqtt_payload))?;
