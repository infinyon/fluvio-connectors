...
async fn simple_test() -> eyre::Result<()> {
    ...
    let postgres_url = std::env::var("FLUVIO_PG_DATABASE_URL").expect("No FLUVIO_PG_DATABASE_URL environment variable found");
    ...
    let _ = admin.create(fluvio_topic.clone(), false, TopicSpec::Computed(TopicReplicaParam::new(1, 1, false))).await;
    ...
    let mut connector = PgConnector::new(config).await.expect("PgConnector failed to initialize");
    let _stream = fluvio_future::task::spawn(async move {
        connector.process_stream().await.expect("process stream failed");
    });
    let (pg_client, conn) = postgres_url
        ...
        .await?;
    tokio::spawn(conn);
    let table_create = r#"CREATE TABLE IF NOT EXISTS foo(ID SERIAL PRIMARY KEY, NAME TEXT NOT NULL)"#;
    ...
    for i in 1..10 {
        ...
        let next = stream.next().await.expect("Failed to get next fluvio steram")?;
        let next = next.value();
